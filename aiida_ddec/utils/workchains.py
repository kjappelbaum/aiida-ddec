...
def extract_core_electrons(cp2k_remote_folder):
    ...
    for n_line, line in enumerate(content):
        if line.startswith(" CP2K| version string:"):
            cp2k_version = line.split()[5]
            cp2k_major_version = int(cp2k_version.split(".")[0])
        if "- Atoms:" in line:
            n_atoms = int(line.split()[2])
        if "Atom  Kind  Element       X           Y           Z" in line:
            break
    if cp2k_major_version >= 9:
        res = {
            e.split()[3]: round(float(e.split()[7]))
            for e in content[
                n_line + 1 : n_line + n_atoms + 1
